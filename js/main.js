document.addEventListener('DOMContentLoaded', function() {
    ...
    // וידוא שהאלמנטים קיימים לפני הוספת אירועים
    if (personalCube && professionalCube && coupleCube) {
        // יצירת מאזיני אירועים לקוביות
        personalCube.addEventListener('click', function() {
            if (personalForm) showForm(personalForm);
            setActiveCube(personalCube);
        });
        
        professionalCube.addEventListener('click', function() {
            if (professionalForm) showForm(professionalForm);
            setActiveCube(professionalCube);
        });
        
        coupleCube.addEventListener('click', function() {
            if (coupleForm) showForm(coupleForm);
            setActiveCube(coupleCube);
        });
    } else {
        console.error('אחד או יותר מהקוביות לא נמצאו בדף.');
    }
    
    // טיפול בהגשת טפסים אם הם קיימים
    if (personalForm) {
...
function showForm(formToShow) {
    // בדיקה שהאלמנט קיים
    if (!formToShow) {
        console.error('הטופס להצגה לא נמצא.');
        return;
    }
    ...
    forms.forEach(form => {
        if (form) {
            ...
            form.style.display = 'none'; // וידוא שהטופס מוסתר
        }
    });
    
    // הצגת הטופס הנבחר
    formToShow.classList.remove('hidden');
    formToShow.style.display = 'block'; // וידוא שהטופס מוצג
...
function setActiveCube(activeCube) {
    // בדיקה שהאלמנט קיים
    if (!activeCube) {
        console.error('הקובייה להפעלה לא נמצאה.');
        return;
    }
...
function handleFormSubmission(form, type) {
    // בדיקה שהטופס קיים
    if (!form) {
        console.error('הטופס להגשה לא נמצא.');
        return;
    }
...
function navigateToResults(data) {
    ...
    switch(data.forecastType) {
        ...
        case 'professional':
            ...
            break;
        case 'couple':
            // שימוש בדף התוצאות הכללי במקרה של תחזית זוגית
